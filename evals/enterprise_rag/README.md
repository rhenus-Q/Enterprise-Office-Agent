...
A lightweight behavioral evaluation harness for the **`enterprise_rag`** graph.
The mocked test suites prove the *code paths* work; this harness measures whether
the system *behaves* well on realistic questions: answering from the corpus,
falling back to the web, declining when it doesn't know, honoring privacy mode,
proving multi-document provenance, and exercising fallback-policy choices.

For the optional Office Agent LLM-assist evals, see
[`../office_agent/llm_assist/`](../office_agent/llm_assist/README.md); the
[`../README.md`](../README.md) navigation page links both areas.
...
| `results.md` | The generated report (overwritten on every full run; not regenerated by schema-only changes). **By default it contains truncated question and answer excerpts** — see [Report privacy](#report-privacy). |
| `enterprise-rag-baseline.md` | Intentionally committed, reviewed baseline snapshot; unlike the generated `results.md`, this file is tracked in Git. |
...
# Privacy mode: omit all question/answer excerpts from the Markdown report
# (use for sensitive/private datasets — see "Report privacy" below)
uv run python evals/enterprise_rag/run_eval.py --no-answer-text

# Run without writing a history record (still renders the delta section)
uv run python evals/enterprise_rag/run_eval.py --no-history
...
uv run python evals/enterprise_rag/run_eval.py --history-dir /path/to/history/
```

## Report privacy

`evals/enterprise_rag/results.md` is generated from whatever dataset you run. **By default it
embeds truncated excerpts of each row's question and generated answer** in the
"Answers (truncated)" section. That is fine for the committed synthetic AcmeCorp
dataset, but it becomes a data-leak risk if you run the harness against private,
internal, or production questions and then commit the report.

Guidelines:

- **Do not commit `results.md` when it was generated from private or production
  data.** The tracked `enterprise-rag-baseline.md` snapshot should only reflect the synthetic dataset. The generated `results.md` is gitignored.
- For sensitive datasets, run with **`--no-answer-text`**. The report then omits
  every question and answer excerpt (rendering an explicit "Answer text omitted
  by privacy setting" placeholder) while keeping all metadata: the Metrics table,
  the delta section, and the per-question results table (row id, pass/fail, stop
  reason, counters, failed checks). Scoring, pass/fail, and history records are
  unaffected — only the Markdown report changes.
- `evals/enterprise_rag/history/*.json` records are **always metadata-only** regardless of this
  flag: they never store question text, answer text, `page_content`, prompts, or
  raw graph state (see [Record schema](#record-schema)).

## History and delta reporting

Each full eval run persists a compact, machine-readable JSON record to
`evals/enterprise_rag/history/`, one file per run. On subsequent runs the harness loads the
...
`evals/enterprise_rag/history/` is **append-only**: the harness only writes new records, never
...
| `--history-dir PATH` | Directory for history records (default: `evals/enterprise_rag/history/`). |
...
Generated history JSON is **gitignored by default** (`evals/enterprise_rag/history/*.json`).
...
```powershell
git add -f evals/enterprise_rag/history/<selected-baseline>.json
