...
| [020](020-module-owned-cli-entry-points.md) | Module-owned CLI entry points | Each module owns its interactive CLI: the Enterprise RAG loop moves from `main.py` to `enterprise_rag/cli.py`, a new `office_agent/cli.py` wraps `answer_office_request()`, and root `main.py` becomes the repository-level Office Agent entry point (`uv run python main.py` launches the Office Agent CLI; the standalone RAG CLI is `uv run python -m enterprise_rag.cli`) — an intentional behavior change, retiring the `from main import …` re-export surface (in-repo tests migrated to `enterprise_rag.graph.formatting`). Documents the future `api/` + `frontend/` target as direction only. No engine behavior change. |
| [021](021-frontend-observability-workspace.md) | Frontend observability workspace and thin FastAPI adapter | One universal three-pane web workspace (`frontend/`, Vite + React + TS) over a thin FastAPI adapter (`api/`, `GET /api/health` + `POST /api/agent/run`) that calls `answer_office_request()` and maps `OfficeAgentResponse` 1:1, adding adapter-measured `duration_ms`, adapter-derived `execution_mode`, Knowledge-Q&A-only `observability`, and backend-resolved `run_settings`. Additive production changes only: the default-`None` `KnowledgeObservability` carry-through, and optional **request-scoped Run Settings** (an `options` object on the run body → `requested` / `effective` / `applicability` / `constraints` on the response), resolved after routing against server policy by the pure `office_agent.run_settings` — precedence `OFFLINE_MODE` > `PRIVACY_MODE` > server flags > per-run request; a request can only restrict; no environment or global state is mutated (concurrent runs stay isolated); `run_settings` is `null` when `options` is omitted (`enterprise_rag/**` zero diff). Read-only server-policy badges stay visually separate from the interactive per-run controls; honest-observability rule (`effective` shown only from the backend; nothing fabricated; no browser-clock dates); localhost-only/no-auth demo scope; keys-free CI gains `tests/api/` and a `frontend` job. |
